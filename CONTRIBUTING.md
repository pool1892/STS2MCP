# Contributing to STS2 Fast CLI Bridge
...
1. The state/action surface is consumed by the fast CLI, downstream forks like TwitchPlaysSTS2, and custom scripts. Coordinated changes are easier than rolled-back ones.
...
- [ ] **Docs updated.** If you added or changed an API field/action, update `docs/raw-full.md`, `docs/raw-simplified.md`, and the fast CLI/skill references when relevant.
...
## The Python fast CLI (`cli/sts2_fast_cli.py`)

In scope for contributions. **Architectural rule: the Python layer is a thin, automation-oriented client for the C# HTTP API. Core game authority stays in C#.**

Concretely, that means:

- The CLI reads and mutates state through HTTP requests against `localhost:15526`.
- Polling, timing logs, no-decision drains, shifted-index handling, and small ergonomic transforms belong in Python.
- Anything that reads game state, mutates game state, or interprets game rules belongs in C#.
- Strategic gameplay behavior belongs in repo-local skills and notes, not hidden inside the CLI.

When in doubt, put game authority in C#. The Python CLI is replaceable; the mod is not.
...
Build instructions, install paths (Windows/macOS/Linux), and fast CLI setup live in the [README](README.md). They're not duplicated here so there's only one source of truth.
